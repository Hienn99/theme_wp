...
class HL_theme_option {
	private $page_title = 'Hiendev Options';
	private $menu_title = 'Hiendev Options';
	private $menu_slug = 'theme-options-w366';
	private $position = 1;
	private $settings_fields = 'w366-theme-option-8965569';
	...
	public function theContentThemeOptionPage(){ //var_dump( hl_get_option('demo_text') ); 
		?>
...
// Test: hl_get_option('twitter_url');
function hl_get_option($field_id){
...
//
function hl_get_full_src($field_id){
	$arr_single_data = hl_get_image_data($field_id);
...
function hl_get_image_data($field_id){
	$arr_field = hl_get_option( $field_id ); 
	if(!$arr_field) return '';
	$type = $arr_field['type'];
 ...
 */
function hl_get_slider_data($field_id){
	$arr_field = hl_get_option( $field_id ); 
	$type = $arr_field['type'];
	if($type=='slider'){
		$arr_data = $arr_field['val']; 
...
*/
function hl_the_slider_html($field_id, $args ){
	$arr_slides = hl_get_slider_data($field_id);
	if($arr_slides){
...
function hl_the_content_field($field_id){
	echo apply_filters('the_content', hl_get_option($field_id) );
}
